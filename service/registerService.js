...
const registerUser = async (data) => {
  try {
    ...
    if (jsonUserData.registrationType === 'individual') {
      recipients.push(jsonUserData.email);
      names.push(jsonUserData.fullName);
    } else {
      jsonUserData.teamMembers.forEach(member => {
    ...
    recipients.forEach((email, index) => {
      const name = names[index] || "Participant"; // Fallback if name is missing
      const teamId = jsonUserData.teamId ? jsonUserData.teamId : "Will be assigned soon"; // Handle missing team ID
      ...
      const body = `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
        ...
        <div style="background-color: #f9f9f9; padding: 15px; margin: 20px 0; border-radius: 8px;">
          <h4 style="margin: 0 0 10px;">Team Identification Number: <strong>${teamId}</strong></h4>
        </div>
